...
def recursive_solve(encoder_outputs, graph_embedding, attention_inputs,
                    ...
                    decoder, attention_decoder, mask_batch, output_lang,):
    ...
    for f_id, f_mask_dict in mask_batch.items():
        for w_id, w_mask_dict in f_mask_dict.items():
            one_hot_mask = torch.ones(batch_size, output_lang.n_words)
            for b_id, b_mask_list in w_mask_dict.items():
                one_hot_mask[b_id, :] = f.one_hot(torch.tensor(b_mask_list), num_classes=output_lang.n_words).sum(dim=0)
    ...
    while (cur_index <= max_index):
        ...
        for i in range(dec_batch[cur_index].size(1) - 1):
            ...
            mute_mask = 1 - ((prev_gt == output_lang.word2index["<E>"]) * (gt == prev_gt)).float()
            mute_mask = mute_mask.cuda()

            if i == 0:
                mask = f.one_hot(gt, num_classes=output_lang.n_words)
            elif i in mask_batch[cur_index]:
                mask = mask_batch[cur_index][i]
            elif gt.bool().any().item() is False:
                mask = f.one_hot(gt, num_classes=output_lang.n_words)
            else:
...
def train_tree(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, num_value_batch, generate_nums,
               ...
               input_lang, output_lang, num_pos, batch_graph, contextual_input, dec_batch, queue_tree, max_index, mask_batch):
...
def evaluate_tree(input_batch, input_length, generate_nums, embedding, encoder, decoder, attention_decoder, reducer,
    ...
    while head <= len(queue_decode) and head <= 100:
        ...
        while True:
            ...
            prediction = attention_decoder(attention_inputs[0], curr_h, attention_inputs[1], one_hot_mask.cuda())


            s = (curr_c, curr_h)
            _prev_word = sorted(mask[0])[prediction[one_hot_mask==1].argmax()]
            _prev_word = torch.tensor([_prev_word], dtype=torch.long)
...
def evaluate_tree_ensemble_beam_search(input_batch, input_length, generate_nums,
                                       ...
                                       input_lang, output_lang, num_value, num_pos, batch_graph, beam_size=5,
                                       max_length=MAX_OUTPUT_LENGTH):
    input_var = torch.LongTensor(input_batch).unsqueeze(1)
