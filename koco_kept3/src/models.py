...
class DecoderRNN(nn.Module):
    def __init__(self, opt, input_size):
        ...
        if self.opt["dropout_de_in"] > 0:
            self.dropout = nn.Dropout(self.opt["dropout_de_in"])
...
class AttnUnit(nn.Module):
    def __init__(self, opt, output_size):
        ...
        self.mask_param = nn.Parameter(torch.ones(output_size).float(), requires_grad=True)
    ...
    def forward(self, enc_s_top, dec_s_top, enc_2, mask=None):
        ...
        if mask is not None:
            h2y = h2y + (mask.cuda() * self.mask_param * 2) - self.mask_param
