...
class Reducer():
    def __init__(self, label_root_path, cst_class_tree_path=None, debug=False):
        ...
        self.range_per_pi = {
            ...
            "itertools": list(range(self.reverse_label_dict["itertools.__loader__"],
                                    self.reverse_label_dict["itertools.zip_longest"] + 1)),
        ...
        self.hard_except_label = ["BaseString", "BaseNumber", "BaseDict", "BaseSet", "ClassDef", "FunctionDef", "With",
                                  "BaseFormattedStringContent", "Try"]
        ...
        if hasattr(typing, '_GenericAlias'):
            ...
            self.sequence_test = lambda x: (type(x) == typing.Sequence) or (
                'Sequence' == x.__name__ if hasattr(x, '__name__') else False)
            self.union_test = lambda x: (type(x) == typing.Union) or (
                '_Union' == type(x).__name__ if hasattr(type(x), '__name__') else False)

    def build_cst_class_tree(self):
        cst_classes = {cst_class: getattr(cst, cst_class) for cst_class in dir(sys.modules["libcst"]) if
                       inspect.isclass(getattr(cst, cst_class))}
        cst_tree_dict = {}
        ...
        while cst_count != len(cst_tree_dict):
            for curr_tree in list(cst_tree_dict.keys()):
                ...
                if hasattr(cst_classes[curr_tree], '__base__'):
                    ...
                    if curr_base[0] == '_' and len(cst_classes[curr_tree].__bases__) > 1 and "libcst" in \
                            cst_classes[curr_tree].__bases__[1].__module__:
                        curr_base = cst_classes[curr_tree].__bases__[1].__name__
                    parent_tree_key = [_tree for _tree in list(cst_tree_dict.keys()) if
                                       curr_base in list(cst_tree_dict[_tree].expand_tree())]
        ...
        if self.sequence_test(direct_attr) or self.union_test(direct_attr):
            for attr_ele in direct_attr.__args__:
                if 'libcst' in attr_ele.__module__:
                    direct_label.extend(
                        [self.reverse_label_dict[node.tag] for node in self.cst_class_tree.leaves(attr_ele.__name__)])
            ...
            direct_label.extend(
                [self.reverse_label_dict[node.tag] for node in self.cst_class_tree.leaves(direct_attr.__name__)])
    ...
    def get_candidate(self, parent_label, direct_attr, prev_pred_label):
        ...
        if hasattr(cst, curr_latest_label):
            ...
            if not self.has_child(curr_latest_label):
                ...
                child_direct_label = self.flatten_cst_type(
                    self.get_child_node(curr_latest_label)[curr_latest_label_child_num][1])
    ...
    def reduce_out(self, parent_id_list, parent_child_idx, prev_pred_list):
        ...
        prev_pred_label = [tuple_lister(itemgetter(*[str(v) for v in inner_v])(self.label_dict)) for inner_v in
                           prev_pred_list]
    ...
    def test_label_mask(self, label_seq, parent_label=None, child_idx=None, queue_id=0):
        ...
        for id, node in enumerate(label_seq):
            ...
            prev_pred_list.append(
                [self.reverse_label_dict[x] if type(x) is not list else self.reverse_label_dict["<IE>"] for x in
                 label_seq[:id]])
